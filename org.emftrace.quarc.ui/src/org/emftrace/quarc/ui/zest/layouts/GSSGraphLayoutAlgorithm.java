...
package org.emftrace.quarc.ui.zest.layouts;
 ...
 *
 */
public class GSSGraphLayoutAlgorithm extends AbstractLayoutAlgorithm
{    
     ...
     * the constructor
     */
    public GSSGraphLayoutAlgorithm(int styles, final Graph graph)
    {
        super(styles);
        this.graph = graph;
    ...
    private String formatCaption(String text, int height){
    	String result = "";
     ...
     * create the layout
     * 
     * @param nodes a List with all GSSElementGraphNodes of the Graph
     */
    private void createGSSLayout(List<GSSElementGraphNode> nodes)
    {
    	
    	//find the highest sublevel of each level
    	sublevelCount = new HashMap<Integer, Integer>();
    	for ( GSSElementGraphNode node : nodes){
    		if (!(sublevelCount.containsKey(node.getLevel())) ||  node.getSubLevel() +1 > sublevelCount.get(node.getLevel()) ){
    	...
    	List<List<GSSElementGraphNode>> levels = new ArrayList<List<GSSElementGraphNode>>();
        for(int i = 0; i < nodes.size(); i++)
        {
            int totalLevel = totalLevelOffset.get(nodes.get(i).getLevel())+nodes.get(i).getSubLevel(); //sum of sublevels of each level over the current level of the Node + the current sublevel of the Node

            if( levels.size() <= totalLevel )
            {
                int requiredLevels = totalLevel - levels.size() + 1;
                for(int j = 0; j < requiredLevels; j++)
                    levels.add(new ArrayList<GSSElementGraphNode>());
                
                levels.get(totalLevel).add(nodes.get(i));
            }
            else levels.get(totalLevel).add(nodes.get(i));
        }
        
        highestNumberOfNodes = calcHighestNumberOfNodes(levels);
        longestX = calcLongestX(levels); 
		longestY = calcLongestY(levels);
    ...
    private int calcLongestY(List<List<GSSElementGraphNode>>  levels){
    	 int longestY = 0;
     ...
     * @param levels a level hierarchy
     */
     ...
     * @param levels a level hierarchy
     */
    private void adjustLevels(List<List<GSSElementGraphNode>> levels)
    {        
        ...
        {            
            ...
            {
                int additionalXOffset = ((highestNumberOfNodes * (lengthOffset + xOffset)) - (levels.get(i).size() * (lengthOffset + xOffset)))/2;

                int xPos = leftSpace+additionalXOffset + (j * (xOffset + lengthOffset));
                int yPos = topSpace +(totalLevelOffset.get(levels.get(i).get(j).getLevel())+levels.get(i).get(j).getSubLevel()) * (longestY+yOffset);

                levels.get(i).get(j).setLocation(xPos, yPos);
            }
        }
    }
 private int calcHighestNumberOfNodes(List<List<GSSElementGraphNode>> levels) {
...
// search for the longest node label:
	private int calcLongestX(List<List<GSSElementGraphNode>> levels) {
		int longestX = 0;
        for(int i = 0; i < levels.size(); i++)           
            for(int j = 0; j < levels.get(i).size(); j++)
